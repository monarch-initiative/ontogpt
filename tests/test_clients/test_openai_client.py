...
- name: ornithine biosynthesis
  description: The biological process that results in the output of ornithine, ornithine, an amino acid only rarely found in proteins, but which is important in living organisms as an intermediate in the reactions of the urea cycle and in arginine biosynthesis
  subclass_of: 
...
CODE_PROMPT_EXTRACT = """
- abstract: >-
     Amelogenesis imperfecta type 1K (AI1K) is characterized by hypoplastic enamel of all teeth. In some individuals, the pulp chambers may be enlarged and some molars may exhibit taurodontism
     Amelogenesis imperfecta segregated as an autosomal dominant trait in the family described by Smith et al.
     amelogenesis imperfecta type IK (AI1K) is caused by heterozygous mutation in the SP6 gene (608613) on chromosome 17q21.
   ...
   phenotypes:
     ...
     - taurodontism
- abstract: >-
    Familial hemiplegic migraine-2 (FHM-2) is a rare subtype of familial hemiplegic migraine (FHM) characterized by migraine with aura, 
...
class TestCompletion(unittest.TestCase):
    ...
    def test_code_completion_generalization(self):
        ...
        client = OpenAIClient(engine=engine, max_tokens=300)
    ...
    def test_extract_via_code_completion(self):
        ...
        engine = "code-davinci-002"
        # engine = "code-cushman-001"
        client = OpenAIClient(engine=engine, max_tokens=1000)
